...
(function($) { 
    $.fn.jChinaArea = function(o) {
		...
		sProvince.empty();
		sProvince.append("<option value=''>省/直辖市</option>");
		sCity.empty();
		sCity.append("<option value=''>城市</option>");
		sCounty.empty();
		sCounty.append("<option value=''>请选择区</option>");
	...
	sCity.change(function() {
		sCounty.empty();
		sCounty.append("<option value=''>选择县城</option>");
		loc.fillOption(sCounty , '0,' + sProvince.val() + ',' + sCity.val());
		if(o.aspnet){
			writeInput();
		}
