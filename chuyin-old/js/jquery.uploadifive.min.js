...
//								d.error("FORBIDDEN_FILE_TYPE", k);
							...
							if (f.fileType == 'audio/mpeg') {
								var typeArr = f.fileType.split('/');
								f.fileType = typeArr[0];
							}
							if (f.fileType == 'zip') {
								var fileName = $('#uploadifive-file_upload_h5-queue .filename').text();
								var uploadFileArr = fileName.split('.');
								uploadFileType = uploadFileArr[uploadFileArr.length-1];
								if (uploadFileType !== f.fileType) {
									alert('文件格式错误');
//									d.error("FORBIDDEN_FILE_TYPE", k);
									a.cancel.call(g, k);
									return ;
							...
							else if (k.type.indexOf(f.fileType) < 0) {
								alert('文件格式错误');
//								d.error("FORBIDDEN_FILE_TYPE", k);
								a.cancel.call(g, k);
								return ;
