...
function workInfoCallback(r){
	if(r.code == 0){
		...
		if(data['urlZip']!=null&&data['urlZip']!=''){
			$('#bannerbox-1').append('<span class="ico-3d"></span>');
			$('.bannerbox-1 img').attr('src',data['urlMini']);
		}else{

            if(data['url'].indexOf('http') == -1){
                data['url'] = 'https://poppro.cn/'+data['url'];
            }
			$('.bannerbox-1 img').attr('src',data['url']);
		}
		if(data['show']==1){
		...
		//作品简介
		$('.detail-desc-txt').html(data['desc']);
		//作品缩略图
		 ...
		 */
		 $("#bannerbox-1").click(function(){
	       ...
	       $("#bannerbox-1 img").css('cursor','pointer');
	}else {
		systemTip('作品不存在');
