					...
					.each(function() {
						...
						var settings = $.extend({
							...
							queueID : false, // The ID of the DOM object to
												...
												// the #)
							queueSizeLimit : 1, // The maximum number of
						...
						var swfUploadSettings = {
							...
							upload_url : settings.uploader+'?sessionId='+cookie('sessionId')+'&Uploadtype='+queryString('type')+'&workId='+queryString('id'),
		...
		onSelect : function(file) {
			...
			for ( var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					/*var replaceQueueItem = confirm('文件"'
							+ file.name
							+ '"已经上传过了');
...
//						、this.queueData.errorMsg = '您已经上传'
				...
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					...
					this.queueData.errorMsg = '上传失败，文件大小超过限制';
					break;
